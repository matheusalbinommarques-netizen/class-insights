			...
			<div class="flex flex-col gap-6 xl:flex-row xl:items-start xl:justify-between">
				<div class="max-w-3xl">
					<p class="text-xs font-black uppercase tracking-widest text-slate-500">
						Perfil longitudinal do aluno
					</p>
					<h1 class="mt-3 text-3xl font-black tracking-tight text-slate-950 sm:text-4xl">
						{data.student.name}
					</h1>
					<p class="mt-3 text-base leading-8 text-slate-600">
						Turma <strong class="text-slate-950">{data.classroom.name}</strong>. Esta tela resume a
						trajetoria publicada do aluno, compara com a media da turma e ajuda o professor a sair
						do alerta para uma leitura individual mais clara.
					</p>

					<div class="mt-5 flex flex-wrap gap-3">
						<a
							href={resolve(`/teacher/${data.classroom.id}`)}
